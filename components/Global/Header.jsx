...
export default function Header() {
  ...
  const navItems = [
    { title: "Home", href: "/" },
    {
      ...
      children: [
        ...
        { title: "Ready to Move", href: "/property-listing-buy?filter=ready-to-move" },
        { title: "Nearing Completion", href: "/property-listing-buy?filter=nearing-completion" },
        { title: "Under Construction", href: "/property-listing-buy?filter=under-construction" },
        { title: "Sky Ora Serene Heights", href: "/property-details?id=serene-heights" },
        { title: "Sky Ora Grand Residency", href: "/property-details?id=grand-residency" },
        { title: "Sky Ora Elara", href: "/property-details?id=elara" },
        { title: "Sky Ora Verdant Park", href: "/property-details?id=verdant-park" },
      ],
    },
    { title: "About", href: "/about-us" },
    {
      ...
      href: "#",
      children: [
        { title: "Landowners", href: "/contact-us?subject=landowners" },
        { title: "Channel Partners", href: "/contact-us?subject=channel-partner" },
        { title: "Investors", href: "/contact-us?subject=investors" },
        { title: "Refer & Earn", href: "/contact-us?subject=refer" },
      ],
    },
    {
      title: "Insights",
      ...
      children: [
        ...
        { title: "Article Details", href: "/blog-details?id=1" },
      ],
    },
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 w-full z-[999] uppercase tracking-wider text-[14px] font-medium font-sans transition-all duration-300 ${isSticky
          ? "bg-[#1D1D1D] text-white shadow-[0_2px_15px_rgba(0,0,0,0.15)] border-b border-zinc-800"
          : "bg-[#1D1D1D]/95 backdrop-blur-md text-white shadow-sm"
          }`}
      >
        <div className="relative w-full">
          <div className="container mx-auto px-4 max-w-[1416px]">
            <div
              className={`flex items-center justify-between transition-all duration-300 ease-in-out ${isSticky ? "h-20" : "h-[100px]"
                }`}
            >
              {/* Brand Logo */}
              <div className="flex items-center h-full">
                <Link href="/" aria-label="Home" className="flex items-center">
                  <Image
                    ...
                    height={40}
                    priority
                    className="h-9 w-auto object-contain animate-pulse-slow"
                  />
                </Link>
              </div>
                ...
                <ul className="flex items-center h-full m-0 p-0 list-none gap-6 xl:gap-[40px]">
                  {navItems.map((item, idx) => (
                    <li key={idx} className="group relative h-full flex items-center">
                      {item.children ? (
                        <span
                          className="flex items-center gap-1 py-2 text-white cursor-default select-none"
                        >
                          <span>{item.title}</span>
                          <ChevronDownIcon className="w-3 h-3 text-white transition-transform duration-300 group-hover:rotate-180" />
                        </span>
                      ) : (
                        <Link
                          href={item.href}
                          className="flex items-center gap-1 py-2 text-white hover:text-[#00CC61] transition-colors select-none"
                        >
                          <span>{item.title}</span>
                        </Link>
                      )}
                        ...
                        <ul className="absolute left-0 top-full w-[220px] bg-white shadow-[0_1px_10px_rgba(0,0,0,0.1)] py-4 border-t-2 border-[#1D1D1D] rounded-b-md opacity-0 invisible translate-y-[-10px] group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-300 z-50 flex flex-col list-none m-0">
                          {item.children.map((subItem, subIdx) => (
                            <li key={subIdx} className="w-full">
                              <Link
                                href={subItem.href}
                                className="block px-5 py-2 text-xs normal-case font-medium text-[#1D1D1D] hover:text-[#00CC61] transition-colors"
                              >
                                {subItem.title}
                              </Link>
                            </li>
              ...
              <div className="flex items-center justify-end gap-6 xl:gap-[25px]">
                ...
                >
                  ...
                  <div
                    className={`absolute top-full right-0 w-[120px] bg-white shadow-[0_4px_16px_rgba(2,0,181,0.1)] p-3 rounded transition-all duration-200 z-50 flex flex-col gap-2 ${isLangDropdownOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"
                      }`}
                  >
                ...
                <div className="hidden sm:block">
                  <Link
                    href="/contact-us"
                    className="group/btn relative overflow-hidden inline-flex items-center justify-center gap-2 px-[24px] py-[9px] rounded-[20px] border border-white text-white font-medium text-[16px] capitalize hover:text-black transition-colors cursor-pointer"
                  >
                    <span className="relative z-10 text-white group-hover/btn:text-black transition-colors duration-500">Book a Site Visit</span>
                ...
                >
                  <span
                    className={`w-full h-0.5 bg-white transition-all duration-300 origin-left ${isMobileMenuOpen ? "rotate-45 translate-x-1" : ""
                      }`}
                  />
                  <span
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`fixed inset-0 z-[1000] lg:hidden transition-all duration-300 ${isMobileMenuOpen ? "visible opacity-100" : "invisible opacity-0"
          }`}
      >
        ...
        {/* Sliding Menu Panel */}
        <div
          className={`absolute right-0 top-0 w-full max-w-[320px] sm:max-w-[400px] h-full bg-white shadow-2xl p-8 pt-24 flex flex-col transition-transform duration-500 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
            }`}
        >
                  ...
                  {item.children ? (
                    <div>
                      ...
                      >
                        <span>{item.title}</span>
                        <ChevronDownIcon
                          className={`w-3.5 h-3.5 text-gray-500 transition-transform duration-300 ${mobileSubmenuOpen === idx ? "rotate-180" : ""
                            }`}
                        />
                      </button>
                      ...
                      <div
                        className={`transition-all duration-300 overflow-hidden ${mobileSubmenuOpen === idx ? "max-h-[500px] opacity-100 mt-1" : "max-h-0 opacity-0"
                          }`}
                      >
                          ...
                          {item.children.map((subItem, subIdx) => (
                            <li key={subIdx}>
                              <Link
                                href={subItem.href}
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="block py-2 text-xs normal-case text-gray-600 hover:text-[#00CC61] transition-colors select-none"
                              >
                                {subItem.title}
                              </Link>
                            </li>
                    ...
                    <Link
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="block py-2 font-medium text-sm text-[#1D1D1D] hover:text-[#00CC61] transition-colors select-none"
                    >
                      {item.title}
                    </Link>
                  )}
          ...
          <div className="flex flex-col gap-4 border-t border-gray-100 pt-6">
            {/* Login */}
            <span
              className="text-center py-2.5 font-medium text-sm capitalize text-[#1D1D1D] border border-gray-200 rounded-[20px] cursor-default select-none"
            >
              Login
            </span>
            ...
            <Link
              href="/contact-us"
              onClick={() => setIsMobileMenuOpen(false)}
              className="group/btn relative overflow-hidden inline-flex items-center justify-center gap-2 px-[24px] py-[11px] rounded-[20px] border border-[#1D1D1D] text-white bg-[#1D1D1D] font-medium text-[16px] capitalize hover:text-white transition-colors cursor-pointer select-none"
            >
